...
const fetchTripInfo = async (vehicleId) => {
  ...
  const getSeatName = (seat) => {
    if (seat.status === 'Booked') {
      return 'text-red-500 cursor-not-allowed'; // Ghế đã đặt
    }
    if (seat.status === 'Selected') {
      return 'text-emerald-600 cursor-not-allowed'; // Ghế đã chọn (sau thanh toán)
    }
    if (selectedSeats.includes(seat.id)) {
      return 'text-emerald-600 cursor-pointer'; // Ghế đang được chọn tạm thời
    }
  ...
  return (
    <div className="w-full grid grid-cols-5 gap-10">
      ...
      <div className="col-span-2 w-full space-y-5 bg-neutral-50 rounded-xl py-4 px-6 border border-neutral-200 shadow-sm">
        ...
        <div className="w-full space-y-2">
          <h1 className="text-lg text-neutral-600 font-medium">Fare Detail</h1>
