...
export class TicketController {
  ...
  async getSeatInfo(@Body() body: { tripId: string; seatNumber: string[] }) {
    ...
    return this.ticketsService.getTicketInfoBySeat(
      body.tripId,
      body.seatNumber,
    );
  }
  ...
  async holdSeat(
    ...
    if (!body.tripId || !body.seatNumber || !body.username || !body.vehicleId) {
      throw new BadRequestException(
        'tripId, seatNumber, username, and vehicleId are required',
      );
    }
    ...
    try {
      ...
      if (unavailableSeats.length > 0) {
        throw new BadRequestException(
          `Seats ${unavailableSeats.join(', ')} are already taken for this vehicle`,
        );
      }
